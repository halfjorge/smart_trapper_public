...
(function () {
  ...
  function findSamplePointByScan(doc, scanStep){
    ...
    var found = null;
    for(var y=T+1; y<=B-2; y+=scanStep){
      for(var x=L+1; x<=R-2; x+=scanStep){
        if(testPoint(x,y)){ found = [x,y]; break; }
      }
      if(found) break;
    }
  ...
  try{
    ...
    folder = Folder.selectDialog("Select JOB folder (contains job.json + traps.json + traps/)");
    if(!folder) return;
